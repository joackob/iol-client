...
class IOLClient:
    ...
    async def _request(
        self, method, url, data_body=None, json_body=None
    ...
    async def post_asesores_test_inversor(
        self, respuesta_inversor, id_cliente_asesorado=None
    ):
        path = "asesores/test-inversor"
    ...
    async def get_fci_tipo_fondos_administradoras(
        self, administradora, tipo_fondo=None
    ):
        path = f"titulos/fci/administradoras/{administradora}/tipofondos"
        if tipo_fondo:
            path = (
                f"titulos/fci/administradoras/{administradora}/tipofondos/{tipo_fondo}"
            )
        return await self._request("GET", path)
